...
def _read_parquet_row_group(
    ...
    *args
    ...
    df, views = _pre_allocate(rg.num_rows, columns, categories, index, cs, dt, tz)
    read_row_group_file(
        fn,
        rg,
        columns,
...
def _get_pyarrow_divisions(pa_pieces, divisions_name, pa_schema, infer_divisions):
    ...
    if infer_divisions is True:
        ...
        if divisions_name_in_schema is False:
            if divisions_name is None and pa.__version__ >= LooseVersion("0.13.0"):
                # pyarrow < 0.13 does not expose num_rows
                divisions = [0]
                for piece in pa_pieces:
                    divisions.append(divisions[-1] + piece.get_metadata().num_rows)
...
def _read_pyarrow_parquet_piece(
    fs, piece, columns, index_cols, is_series, partitions, categories
):
    import pyarrow as pa

    with fs.open(piece.path, mode="rb") as f:
        table = piece.read(
            columns=index_cols + columns,
...
_pyarrow_write_metadata_kwargs = {
    "version",
    "use_deprecated_int96_timestamps",
    "coerce_timestamps",
}
...
def _write_pyarrow(
    ...
    **kwargs
    ...
    if append:
        raise NotImplementedError("`append` not implemented for " "`engine='pyarrow'`")

    if ignore_divisions:
        raise NotImplementedError(
            "`ignore_divisions` not implemented for " "`engine='pyarrow'`"
        )
    ...
    if set(kwargs).difference(_pyarrow_write_table_kwargs):
        msg = "Unexpected keyword arguments: " + "%r" % list(
            set(kwargs).difference(_pyarrow_write_table_kwargs)
        )
        raise TypeError(msg)
...
def get_engine(engine):
    ...
    if engine == "auto":
        ...
        _ENGINES["fastparquet"] = eng = {
            "read": _read_fastparquet,
        ...
        if pa_version < "0.8.0":
            ...
            raise RuntimeError(
                "PyArrow version 0.13.0 isn't supported, please " "upgrade or downgrade"
            )

        _ENGINES["pyarrow"] = eng = {"read": _read_pyarrow, "write": _write_pyarrow}
        return eng
